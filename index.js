...
console.log("ilk fenomenin profile ismi : ", fenomenler[0].profile);

//(2) Dizideki üçüncü fenomenin (2. dizin) takipçi (followers) sayısı
console.log("ücüncü fenomenin takipci sayısı : ", fenomenler[3].followers);
...
console.log("düzeltilmiş hali", fenomenler[6].profile);
...
function indekseGoreFenomen(fenom1, index) {
  return `${fenom1.indexOf(fenom1[index])}. indekste bulunan fenomen: ${
    fenom1[index].profile
  }`;
}
...
function profilListesi(feno1) {
  let fenomenn = [];
  for (let i = 0; i < feno1.length; i++) {
    fenomenn.push(feno1[i].profile);
  }
  return fenomenn;
}
...
function fenomenEkle(eklenecekArray, birinci, ikinci, ucuncu, dorduncu, besinci) {
  let copyA = [...eklenecekArray];
  let ekleA = {
    number: birinci,
    profile: ikinci, 
    followers: ucuncu, 
...
function enFenomenler(fenom) {
  let enfon = [];
  for (let i = 0; i < fenom.length; i++) {
    if (fenom[i].followers >= 100000000) {
      enfon.push(fenom[i].profile);
        
    }
  }return enfon
  
}
console.log("100 milyon takipcili hesaplar : ", (fenomenler));
...
function fenomenGonderimSayisi(fenom, pro){
  for (let i=0 ; i < fenom.length; i++ ) {
    if (fenom[i].profile == pro) {
      return fenom[i].posts;
    }
  }
  /*kod*/
}
console.log("posts sayisi : ", fenomenGonderimSayisi(fenomenler, "Will Smith"));
...
Örnek: platformaGoreCokGonderiYapanFenomen(fenomenler, 'TikTok') çağrıldığında "charli damelio" dönmelidir
 bütün indexleri döndür. plat form yazılansa number 1 i seç */

 function platformaGoreCokGonderiYapanFenomen(feno, plot) {
  let maxPost = 0;
  let Fenomen = " ";
  for (let i = 0; i < feno.length; i++) {
    if (
      feno[i].platform == plot &&
      feno[i].posts != "NA" &&
      feno[i].posts > maxPost
    ) {
      Fenomen = feno[i].profile;
      maxPost = feno[i].posts;
    }
  }
  return Fenomen;
}
